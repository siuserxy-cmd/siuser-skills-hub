...
同时可作为一个完整 Skill 使用，覆盖“生成文案 → 规划配图 → 输出提示词 / 批量出图 → 发布”。

## 功能特性
- **自动化发布**：自动填写标题、正文、上传图片
- **Skill 文案生成**：根据主题/链接生成理性干货风小红书文案
- **Skill 配图工作流**：把文案拆成图清单、Copy Spec 和可复制的出图提示词
- **批量出图**：内置 `scripts/apimart_batch_generate.py`，支持 APIMart 批量出图与 dry-run
...
如需批量出图，额外在本地创建 `scripts/apimart.env`，格式可参考 `scripts/apimart.env.example`。
...
本项目可作为 Claude Code、OpenCode 等支持 Skill 的工具使用。当前 Skill 已整合：
...
如果只想用 Skill，不必手写完整命令；可以直接提需求，例如：

```text
帮我写一篇 AI 工具推荐的小红书文案，并给 4 张配图提示词
```

```text
把这篇文案直接配图，生成图片后再准备发布
```
...
本项目公开分发时应保留原始许可证与第三方说明。

- 原始许可见 [LICENSE](LICENSE)

...
def normalize_requests(
    ...
    *,
    ...
    for raw in raw_requests:
        ...
        if provider == "kie":
            # KIE 4o image API uses fixed endpoint semantics and supports 1:1 / 3:2 / 2:3.
            normalized_size = {"3:4": "2:3", "4:3": "3:2", "16:9": "3:2", "9:16": "2:3"}.get(size, size)
            payload = {
                "prompt": joined_prompt,
                "size": normalized_size,
                "nVariants": n_value,
                "isEnhance": bool(raw.get("isEnhance", False)),
            }
        else:
            payload = {
                "model": model,
...
def _extract_urls(obj: Any) -> list[str]:
    ...
    def visit(node: Any) -> None:
        if isinstance(node, dict):
            for key in ("url", "urls", "resultUrl", "resultUrls", "result_url", "result_urls"):
                value = node.get(key)
                if isinstance(value, str) and value.startswith(("http://", "https://")):
                    urls.append(value)
...
def poll_task(
    *,
    ...
    if provider == "kie":
        url = f"{base_url}/api/v1/gpt4o-image/record-info?taskId={parse.quote(task_id, safe='')}"
    else:
    ...
    while True:
        ...
        data = (last_obj or {}).get("data") or {}
        status = str(data.get("status") or "").strip().lower()
        success_flag = data.get("successFlag")
        if provider == "kie":
            if success_flag in {1, 2} or status in {"success", "create_task_failed", "generate_failed"}:
                break
...
def main(argv: list[str]) -> int:
    ...
    if not provider:
        provider = "kie" if "api.kie.ai" in api_url else "apimart"
